...
public class PredictionService {
    ...
    public PredictionAnalyze analyze(
        ...
        switch (ctx.bet()) {
            case OVER_ZERO_FIVE: {
                var goals =
                        ctx.match().getResultAwayTeamGoals() +
    ...
    public PredictionResult prediction(BetPredictionContext ctx) {
        ...
        for (BetResultInfluencer influencer : betResultInfluencer) {
            ...
            if (preCheckResult == PrecheckResult.OK) {
                ...
                influencerDetailedResult.add(new InfluencerResult(influencer.influencerName(), predictionInfluence, preCheckResult));
            } else if (
        ...
        return new PredictionResult(result, betOnThis, analyze(ctx, doneInfluencerCalculations > 0, betOnThis), influencerDetailedResult);
    }
