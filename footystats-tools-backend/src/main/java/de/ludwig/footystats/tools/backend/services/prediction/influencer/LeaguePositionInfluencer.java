...
public class LeaguePositionInfluencer implements BetResultInfluencer {
    public PrecheckResult preCheck(BetPredictionContext ctx) {
        ...
        if (
                ArrayUtils.contains(new Bet[]{Bet.OVER_ONE_FIVE, Bet.OVER_ZERO_FIVE}, ctx.bet()) == false
        ) {
    ...
    private Float overBet(BetPredictionContext ctx){
		...
		const awayPos = teamStats.league_position_away; // TODO teamStats ist aktull ein Array (auch im alten Code). Das hier kann so also nicht stimmen.
